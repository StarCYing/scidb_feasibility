...
local dataLocation = "https://ntia.doc.gov/data/cps/stata"
local codeLocation = "https://ntia.doc.gov/data/cps/analyzetable"
local location numDatasets
tempname output
tempfile generatedTables

* Variables used in analyze table
local demographics "ageGroup workStatus income education sex race disability metro schoolChildrenAtHome gestfips"
...
if "`location'" == "local" {
	...
	local dataLocation = "O:\CPS Computer and Internet Use Supplement\Stata"
	local codeLocation = "O:\CPS Computer and Internet Use Supplement\Create Analyze Table"
}
...
foreach x of numlist 1/`numDatasets' {
	...
	preserve
	keep if isHouseholder
	capture confirm variable hhwgt1
	if _rc == 0 {
		svyset [iw=householdWeight], sdrweight(hhwgt1-hhwgt160) vce(sdr)
	}
	else {
		svyset [iw=householdWeight]
	}
	foreach q in isHouseholder $householdVars {
	...
	if _rc == 0 {
		svyset [iw=personWeight], sdrweight(pewgt1-pewgt160) vce(sdr)
	}
	else {
	...
	foreach q in isPerson isAdult $personVars {
		* pull overall numbers
		svy: mean `q'
		matrix temp = e(b)
		local theStat = temp[1,1]
	...
	if _rc == 0 {
		...
		if _rc == 0 {
			svyset [iw=respondentWeight], sdrweight(rewgt1-rewgt160) vce(sdr)
		}
		else {
			svyset [iw=respondentWeight]
		}
		foreach q in $respondentVars {
			* pull overall numbers
			svy: mean `q'
			matrix temp = e(b)
			local theStat = temp[1,1]
...
macro drop $householdVars $personVars $respondentVars
