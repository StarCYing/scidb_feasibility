...
ntia_estimate <- function(feature, basewgt, repwgt){
    if(feature[1] %in% c(TRUE, FALSE)){
        feature_cor = feature * 1
    } else {
        feature_cor = feature
    }
    mean_feat = mean(feature_cor * basewgt, na.rm=TRUE)
    mean_replicates = apply(feature_cor * repwgt, 2, mean)
    mean_se = ((4/160)*sum((mean_feat - mean_replicates)^2))^.5
    sum_feat = sum(feature_cor * basewgt, na.rm=TRUE)
    sum_replicates = apply(feature_cor * repwgt, 2, sum)
    sum_se = ((4/160)*sum((sum_feat - sum_replicates)^2))^.5
    return(cbind(mean_feat,mean_se[],sum_feat,sum_se[]))
}

filter_demo <- function(demo, cond){
    uniq = unique(demo)
    demo_ind = lapply(uniq, function(x) which(demo %in% c(x)))
    names(demo_ind) = uniq
    return(demo_ind)
}
...
ntia_demo_est <- function(feature, basewgt, repwgt, demo){
    ...
    demo_ind = filter_demo(demo)
    mean_feat = sapply(demo_ind, function(x) mean((feature * basewgt)[ x ]))
    # mean_feat = aggregate(feature * basewgt, by=list(Category=demo), FUN=mean, na.rm=TRUE)
    mean_rep = sapply(demo_ind, function(x) apply((feature_cor * repwgt)[ x, ], 2, mean)[])
    # mean_rep = aggregate(feature * repwgt, by=list(Category=demo), FUN=mean, na.rm=TRUE)
    mean_se = sapply(1:length(demo_ind), function(x) ((4/160) * sum((mean_feat[x] - mean_rep[ ,x])^2))^.5)
    names(mean_se) <- colnames(mean_rep)
    # mean_se = ((4/160) * rowSums(mean_feat[,2] - mean_rep[,2:161])^2)^.5
    sum_feat = sapply(demo_ind, function(x) sum((feature * basewgt)[ x ]))
    # sum_feat = aggregate(feature * basewgt, by=list(Category=demo), FUN=sum, na.rm=TRUE)
    sum_rep = sapply(demo_ind, function(x) apply((feature_cor * repwgt)[ x, ], 2, sum)[])
    # sum_rep = aggregate(feature * repwgt, by=list(Category=demo), FUN=sum, na.rm=TRUE)
    sum_se = sapply(1:length(demo_ind), function(x) ((4/160) * sum((sum_feat[x] - sum_rep[ ,x])^2))^.5)
    names(sum_se) <- colnames(sum_rep)
